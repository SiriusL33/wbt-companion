/*
 * WBT Explorer - Explore Page Logic (FINAL POLISH - TYPO FIX)
 */
...
function renderHeroSection(items) {
    ...
    const posterStyle = (wbt, isBig) => {
        ...
        const bgStyle = `background: linear-gradient(135deg, ${color}08 0%, #ffffff 100%); border: 1px solid ${color}20;`;
        
        return `
        <div class="relative group rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer p-6 flex flex-col justify-between h-full bg-white"
                style="${bgStyle}"
            ...
            <div class="flex justify-between items-center z-10 mb-4">
                ...
                <div class="w-10 h-10 rounded-full bg-white flex items-center justify-center text-[${color}] shadow-sm group-hover:scale-110 transition-transform">
                    ➜
                </div>
            </div>
            ...
            <div class="z-10 mt-auto">
                <h3 class="${titleSize} font-bold text-slate-800 leading-snug mb-2 pb-1 line-clamp-2 group-hover:text-[${color}] transition-colors">
                    ${wbt.title}
                </h3>
                
                ${isBig ? `<p class="text-slate-500 line-clamp-2 text-lg mb-3 font-medium leading-relaxed pb-2">${wbt.description}</p>` : ''}
                
                <div class="flex items-center gap-2 text-xs font-semibold text-slate-400">
                    <span class="bg-white/50 px-2 py-1 rounded border border-slate-100 flex items-center gap-1">
